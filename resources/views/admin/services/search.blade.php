...
@section('title', 'Search for services: ' . request('q', '') . ' |')
            ...
            <div class="row">
                <div class="col-md-4 col-lg-3">
                    ...
                    <div class="box-content-default collapse show" id="collapseOne" aria-labelledby="headingOne" data-bs-parent="#accordionExample">
                        <x-admin.side-bar-search :results="$results" :categories="$categories"
                        :methods="$methods" :languages="$languages" :countries="$countries"
                        :aimeds="$aimeds" :minPrice="$minPrice" :maxPrice="$maxPrice" :subcategories="$subcategories" :types="$types" />
                    </div>
                </div>
                ...
                <div class="col-md-8 col-lg-9">
                    <div class="box-content-default">
                        <div class="row">
                            ...
                            @empty
                                <h5 class="text-center">Ops, we couldn’t find this service in our database yet :(</h5>
                                <!-- Newsletter -->
...
<script>
    ...
    let defaulDateFormat = document.querySelector(".flatpickr").getAttribute('data-default-date').replace(/\s/g, '').split(",");
    const form = document.querySelector("#search-form")

    flatpickr(".flatpickr", {
        mode: "range",
        dateFormat: "d/m/Y",
        defaultDate: defaulDateFormat,
        minDate: "today",
        conjunction: " to ",
        onChange: function(selectedDates, dateStr, instance) {
            let startDate = selectedDates[0] ? `${selectedDates[0].getDate()}-${selectedDates[0].getMonth() + 1}-${selectedDates[0].getFullYear()}` : ''
            let endDate = selectedDates[1] ? `${selectedDates[1].getDate()}-${selectedDates[1].getMonth() + 1}-${selectedDates[1].getFullYear()}` : ''
