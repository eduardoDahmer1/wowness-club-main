...
<a href="{{route('contents.show', $content->slug)}}" class="box_cat_rooms">
    <figure>
        <div class="box-categories">
            @if ($content->categories->count())
                <h6 class="tagsstyle">
                    <img style="max-width: 18px;" src="{{ asset('storage/'.$content->categories[0]->icon) }}" alt="">
                    {{ $content->categories[0]->name }}
                </h6>
            @endif
            @if ($content->subcategories->count())
                <h6 class="tagsstyle">
                    <img style="max-width: 18px;" src="{{ asset('storage/'.$content->subcategories[0]->icon) }}" alt="">
                    {{ $content->subcategories[0]->name }}
                </h6>
            @endif
        </div>
        <div class="background-image" data-background="url({{asset('storage/'. $content->thumbnail)}})"></div>
    </figure>
    <div class="info px-3">
        <div class="d-flex pb-2">
            <div class="box-tags-card">
                <img src="{{$content->cost->icon()}}" alt="">
                <p>{{ $content->cost->name }}</p>
            </div>
            <div class="box-tags-card">
                <img src="{{$content->type->icon()}}" alt="">
                <p>{{ $content->type->name }}</p>
            </div>
            <div class="box-tags-card">
                <img src="{{$content->aimed->icon()}}" alt="">
                <p>{{ $content->aimed->name }}</p>
            </div>
        </div>
        <h3>{{$content->title}}</h3>
        <h6 class="fst-italic fw-light text-muted">{{ str($content->subtitle)->limit(40, "...") }}</h6>
        @if($content->city)
        <div class="d-flex">
            ...
            {{ str($content->city . ', ' . $content->state)->limit(28,"...") }}
        </div>
        @endif

        @if ($content->user->photo && $content->user->alias && $content->user->headline)
        <div class="box-thumb-facilitator">
            <img src="{{asset('storage/'. $content->user->photo)}}" loading="lazy" alt="">
            <div>
                <h6 class="m-0">{{ str($content->user->alias)->limit(20, "...") }}</h6>
                <small class="headline">{{ str($content->user->headline)->limit(33, "...") }}</small>
            </div>
        </div>
        @endif
        @if ($content->cost->name == 'Paid')
            <h4 class="price"><small style="display: inline">From</small>&#163 {{ $content->price }}</h4>
        @else
            <h4 class="price"><small style="display: inline">From</small>FREE</h4>
        @endif
