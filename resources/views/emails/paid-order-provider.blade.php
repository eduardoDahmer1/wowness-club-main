            ...
            <table bgcolor="#F8F9FA" width="600px" cellspacing="0" cellpadding="0" align="center" style="border: none; border-left: 1px solid #6E9D72; border-right: 1px solid #6E9D72; border-radius: 10px;">
              ...
              <tr>
                <td style="font-size: 22px; font-weight: lighter;" align="center">
                  <h3 style="font-weight: lighter; text-align: left; max-width: 460px; font-size: 22px;">Good news <span>{{$firstname}},</span></h3>
                  <p style="max-width: 460px; text-align: justify;">You have received an order for the {{$order->package->service->name}} on the {{$order->package->service->start->format('d/m/Y')}}.</p>
                  <img style="width: 460px; height: 330px; border-radius: 10px;" src="{{ asset('storage/' . $order->package->service->photo) }}" alt="">
                </td>
              </tr>
                  ...
                  <table align="center" width="460px" cellspacing="0" cellpadding="0" style="border: none;">
                    <tr style="font-size: 18px; font-weight: bold; text-transform: uppercase;">
                      ...
                      <td width="150px" align="left">
                        <span style="text-align: left;">{{$order->quantity}} Ticket</span>
                      </td>
                      ...
                      <td width="150px" align="right">
                        <span>{{ number_format((float) $order->package->price * $order->quantity, 2,'.', '') }}</span>
                      </td>
                    </tr>
                  </table>
                </td>
                ...
                <td>
                  <table align="center" width="460px" cellspacing="0" cellpadding="0" style="border: none;">
                    <tr style="font-size: 15px; font-weight:lighter;">
                      <td>
                        <p><img width="17px" src="https://i.ibb.co/2hj4tbv/calendar-1.png" alt="">
                          Date:
                            @if (
                              $order->package->service->start->format('m') == $order->package->service->end->format('m') &&
                                  $order->package->service->start->format('Y') == $order->package->service->end->format('Y'))
                              {{ $order->package->service->start->format('d M') }} {{ __('From') }} {{ $order->package->service->start->format('H:i A') }} -
                              {{ $order->package->service->end->format('d M, Y') }} - {{ $order->package->service->start->format('H:i A') }}

                                <!-- If the year is the same, it displays the format ex: 01 April - 02 May, 2023 -->
                            @elseif ($order->package->service->start->format('Y') == $order->package->service->end->format('Y'))
                                {{ $order->package->service->start->format('d M') }} {{ __('From') }} {{ $order->package->service->start->format('H:i A') }} -
                                {{ $order->package->service->end->format('d M, Y') }} - {{ $order->package->service->start->format('H:i A') }}

                                <!-- If the dates are completely different, display the format ex: 01 April, 2023 - 10 May, 2024 -->
                            @else
                                {{ $order->package->service->start->format('d M, Y') }} {{ __('From') }} {{ $order->package->service->start->format('H:i A') }} - {{ $order->package->service->end->format('d M, Y H:i A') }}
                            @endif
                        </p>
                        <p><img width="17px" src="https://i.ibb.co/CtvprxN/local-1.png" alt="">
                          {{$order->package->service->complement ?? ''}}, {{$order->package->service->number ?? ''}}, {{$order->package->service->zipcode ?? ''}}, {{$order->package->service->city ?? ''}}, {{$order->package->service->country?->name}}
                        </p>
                      </td>
                    </tr>
                  </table>
              ...
              <tr>
                <td style="padding-top: 10px;" align="center">
                  <p style="max-width: 460px; font-weight: lighter;">Check the order page to see full details.</p>
                </td>
              </tr>
              ...
              <tr>
                <td align="center" style="padding-bottom: 60px; padding-top: 40px;">
                  <!--[if mso]>
                  <v:roundrect xmlns:v="urn:schemas-microsoft-com:vml" xmlns:w="urn:schemas-microsoft-com:office:word" href="{{route('orders.index')}}" style="height:40px;v-text-anchor:middle;width:200px;" arcsize="10%" strokecolor="#6E9D72" fillcolor="#6E9D72">
                  <w:anchorlock/>
                  ...
                  padding: 20px 60px;
                  border-radius: 2px;"
                  href="{{route('orders.index')}}">View Order</a>
                  <!--<![endif]-->
                </td>
              </tr>
              ...
              <tr>
                <td style="padding-bottom: 20px; font-size: 14px; color: #364C66;" align="center">
                  <p>This email was sent to: <span style="color: #6E9D72;">{{$order->package->service->user->email}}</span></p>
