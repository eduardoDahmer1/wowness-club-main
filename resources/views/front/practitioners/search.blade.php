...
@section('title', 'Search for practitioners: ' . request('q', '') . ' |')
            ...
            <div class="row">
                <div class="col-md-4 col-lg-3">
                    ...
                    <div class="box-content-default collapse show" id="collapseOne" aria-labelledby="headingOne" data-bs-parent="#accordionExample">
                        <x-admin.side-bar-search-practitioner :categories="$categories"
                            :languages="$languages" :subcategories="$subcategories" />
                    </div>
                </div>
                    ...
                    <div class="box-content-default">
                        <div class="row">
                            @if (request('q', ''))
                                <div class="title mb-2">
                                    <small data-cue="slideInUp">Practitioners for</small>
                                    <h2 data-cue="slideInUp" data-delay="200">{{ request('q', '') }}</h2>
                                </div>
                            @endif
                            @forelse ($practitioners as $practitioner)
                            <div class="col-xl-4 col-lg-6 cardpractitioners" data-cues="slideInUp" data-delay={{$loop->index + 3 . '00'}}>
                                <x-front.cardpractitioner :$practitioner />
                            </div>
                            @empty
                                <h5 class="text-center">Ops, we couldn’t find this practitioner in our database yet :(</h5>
                                <!-- Newsletter -->
                        ...
                        <div class="card-body text-right">
                            @if ($practitioners->hasPages())
                                {!! $practitioners->withQueryString()->links() !!}
                            @endif
                        </div>
